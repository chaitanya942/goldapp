...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, logoBase64 }) {
  ...
  // Reserve below table: words box (~14mm) + GT box (~10mm) + ~8mm buffer
  const bottomReserve = 32
  const availForTable = sigTextY - y - bottomReserve
  const rH_est   = 5.5    // actual autoTable row height at 7.5pt + 1.8mm cellPadding
  const ftrCount = isInterstate ? 3 : 1
  const tblFixed = 10 + rH_est + ftrCount * rH_est  // header(10) + data(1) + footer rows
  const nEmpty   = Math.max(4, Math.floor((availForTable - tblFixed) / rH_est))

  // Footer rows placed in body — this keeps column lines continuous and
  // inherits columnStyles alignment correctly (foot section ignores columnStyles)
  ...
  autoTable(doc, {
    ...
    didParseCell(d) {
      if (d.section === 'body' && d.row.index >= 1 + nEmpty) {
        // Bold the footer-style rows (VALUE OF GOODS, IGST, GRAND TOTAL)
