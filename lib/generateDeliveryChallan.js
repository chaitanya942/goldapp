...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, logoBase64 }) {
  ...
  // Calculate nEmpty so the table fills the page without overflow
  // Reserve below table: words box (~12mm) + GT box (~10mm) + blank sig space (~35mm) + margin (5mm)
  const bottomReserve = 62
  const availForTable = sigTextY - y - bottomReserve
  const rH_est   = 6.25   // estimated autoTable row height at 7.5pt + 1.8mm cellPadding
  ...
  // Merge the four empty left columns into one cell so there are no stray dividers
  const footR = isInterstate
    ? [
        [{ content: '', colSpan: 4 }, 'VALUE OF GOODS', fmtIN(vog)],
        [{ content: '', colSpan: 4 }, `IGST @ ${igstRate}%`, fmtIN(igst)],
        [{ content: '', colSpan: 4 }, 'GRAND TOTAL',    fmtIN(grand)],
      ]
    : [[{ content: '', colSpan: 4 }, 'GRAND TOTAL', fmtIN(grand)]]
  ...
  autoTable(doc, {
    ...
    // Column widths chosen so "QUANTITY/GROSS" and "VALUE OF GOODS" never wrap:
    // col3 inner = 28-3.6=24.4mm > "QUANTITY/GROSS"@7.5pt≈22mm ✓
    // col4 inner = 28-3.6=24.4mm > "VALUE OF GOODS"@7.5pt≈22mm ✓
    columnStyles: {
    ...
    didParseCell(d) {
      // Footer label (col 4) and amount (col 5) inherit center from columnStyles
      // No overrides needed
    },
