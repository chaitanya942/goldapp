...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, logoBase64 }) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Weight column is "QUANTITY/GROSS WEIGHT (GMS)" — must use gross_weight to
  // match the EWB and E-Invoice payloads (both already use gross). Previously
  // showed net_weight, which silently mismatched the GST documents.
  const totWt  = items.reduce((s, p) => s + parseFloat(p.gross_weight || 0), 0)
