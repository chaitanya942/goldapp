...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, hubGroups, logoBase64 }) {
  ...
  const irLblW = 44
  const ir4 = [
    ['DELIVERY CHALLAN NO',   consignment.challan_no || '',   'TRANSPORTER NAME',   consignment.transporter_name || companySettings.transporter_name   || 'BVC LOGISTICS PVT. LTD.'],
    ['DELIVERY CHALLAN DATE', dateStr,                        'TRANSPORTATION MODE', consignment.transport_mode  || companySettings.transportation_mode || 'BY AIR & ROAD'],
