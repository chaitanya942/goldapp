...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, hubGroups, logoBase64 }) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 4. VERIFICATION LIST — bill-wise (outstation) or branch-wise (Bangalore hub)
  //
  // Outstation challans show one row per bill with App ID + Customer + Date +
  // Branch so the receiving team can tick off each parcel. Per-bill weights
  // and per-bill values are intentionally not shown here — only the
  // consolidated total appears in the GST items table below.
  //
  // Bangalore hub dispatches consolidate every assigned branch (the hub +
  // its leaves) into one consignment. Showing 50–80 bill rows there is
  // operationally noise — the receiving team verifies branch-by-branch.
  // So when hubGroups is supplied the verification table renders one row
  // per source branch with bills count, gross weight, and value. The
  // legal items table (consolidated total) is unchanged.
  // ═══════════════════════════════════════════════════════════════════════════
  if (Array.isArray(hubGroups) && hubGroups.length > 0) {
    ...
    autoTable(doc, {
      ...
      theme: 'grid',
      tableWidth: 190,
      styles:     { fontSize: 7, cellPadding: 1.2, lineColor: BK, lineWidth: 0.15, textColor: BK, valign: 'middle' },
      headStyles: { fillColor: HG, textColor: BK, fontStyle: 'bold', halign: 'center', fontSize: 7, valign: 'middle', cellPadding: 1.2 },
      columnStyles: {
    ...
    doc.setFontSize(6.5); doc.setFont('helvetica', 'italic'); doc.setTextColor(...GY)
    doc.text(
      'Per-bill weights and values are confidential — see the consolidated total in the GST items table below.',
      L, y + 2.5
    )
    y += 5
  }
  ...
  // Reserve: bill verification list above already consumed space; reserve only
  // what's strictly needed below: manual footer rows + words box + GT + sig gap.
  ...
  // ── AUDIT FINGERPRINT ────────────────────────────────────────────────────
  // Same 8-char hash printed on the Consignee Report / Issue Voucher and
  // exposed in the document-audit response. If hashes match across all
  // documents in hand, every audited field agrees.
  ...
  doc.text(`AUDIT HASH:  ${auditHash}`, L, sigTextY + 8)
  doc.setFontSize(6.5); doc.setFont('helvetica', 'italic'); doc.setTextColor(...GY)
  doc.text('Cross-check this hash against the Consignee Report and Issue Voucher / EWB / E-Invoice. All should match.',
    L + useW, sigTextY + 8, { align: 'right' })
