...
export function generateDeliveryChallan({ consignment, branch, companySettings, items }) {
  ...
  // TAMPER PROOF No:- [normal black] WG000xxx [bold red]
  doc.setFontSize(9.5)
  ...
  infoRows.forEach(([ll, lv, rl, rv], i) => {
    ...
    // Challan NO row: render the last 6-digit ext segment in red
    if (i === 0) {
      const parts  = lv.split('/')
      const extSeg = parts.pop()                        // "001904"
      const prefix = ': ' + parts.join('/') + '/ '     // ": WGKA/KA-TUM/MAR/2026/ "
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...BLACK)
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(...RED)
      doc.text(extSeg, L + irLblW + prefixW, ty)
      doc.setTextColor(...BLACK)
    } else {
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...BLACK)
  ...
  rf('STATE CODE', '29',     rOff + 5)    // HO is always KA = 29
  ...
  // Interstate tax (non-KA branch → Karnataka HO)
  const isInterstate = sc !== 'KA'
  const valueOfGoods = isInterstate ? totalAmount * 1.075 : totalAmount
  ...
  // Footer rows: add IGST row for interstate
