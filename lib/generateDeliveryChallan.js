...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, logoBase64 }) {
  ...
  // Use ' ' (space) in cols 0-3 of footer rows — empty string '' causes
  // autotable grid theme to skip drawing vertical borders for those cells
  const _ = ' '
  const ftrRows = isInterstate
    ? [
        [_, _, _, _, 'VALUE OF GOODS',      fmtIN(vog)],
        [_, _, _, _, `IGST @ ${igstRate}%`, fmtIN(igst)],
        [_, _, _, _, 'GRAND TOTAL',         fmtIN(grand)],
      ]
    : [[_, _, _, _, 'GRAND TOTAL', fmtIN(grand)]]
  ...
  autoTable(doc, {
    ...
    didParseCell(d) {
      if (d.section === 'body' && d.row.index >= ftrStart) {
        ...
        // Explicitly set alignment for every column in footer rows
