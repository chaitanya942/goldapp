...
export function generateDeliveryChallan({ consignment, branch, companySettings, items, logoBase64 }) {
  ...
  // Reserve below table: words box (~12mm) + GT box (~10mm) + margin (5mm)
  const bottomReserve = 62
  const availForTable = sigTextY - y - bottomReserve
  const rH_est   = 6.25
  const ftrCount = isInterstate ? 3 : 1
  const tblFixed = 8 + rH_est + ftrCount * rH_est
  const nEmpty   = Math.max(4, Math.floor((availForTable - tblFixed) / rH_est))

  // \u00A0 (non-breaking space) in empty footer cells forces autotable to
  // render cell borders — without content autotable skips drawing the dividers
  const E = '\u00A0'
  const footR = isInterstate
    ? [
        [E, E, E, E, 'VALUE OF GOODS',      fmtIN(vog)],
        [E, E, E, E, `IGST @ ${igstRate}%`, fmtIN(igst)],
        [E, E, E, E, 'GRAND TOTAL',         fmtIN(grand)],
      ]
    : [[E, E, E, E, 'GRAND TOTAL', fmtIN(grand)]]
  ...
  autoTable(doc, {
    ...
    footStyles: { fillColor: WH, textColor: BK, fontStyle: 'bold', valign: 'middle', lineColor: BK, lineWidth: 0.2 },
    columnStyles: {
    ...
    didParseCell(d) {
      if (d.section === 'foot') {
        // Re-apply per-column alignment since footStyles overrides columnStyles
        const aligns = ['center', 'left', 'center', 'center', 'center', 'center']
        d.cell.styles.halign = aligns[d.column.index]
      }
    },
