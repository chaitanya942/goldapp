...
/**
 * Find the soonest upcoming shift whose window hasn't closed yet — used by
 * the lock screen to tell the auditor "your next shift opens at X." If the
 * auditor is currently inside a window, that window IS the next-up; we
 * still surface it so the UI can show its closing time.
 *
 * @param {Array<{shift_date: string, shift_type: 'night'|'morning'}>} shifts
 * @param {number} [nowMs]
 * @returns {{
 *   shift_date:   string,
 *   shift_type:   'night' | 'morning',
 *   opensAtMs:    number,    // UTC ms when the window opens
 *   closesAtMs:   number,    // UTC ms when the window closes
 *   windowStartIst: string,  // 'HH:MM'
 *   windowEndIst:   string,  // 'HH:MM'
 * } | null}
 */
export function findNextShift(shifts, nowMs = Date.now()) {
  let best = null
  for (const s of shifts || []) {
    const w = windowFor(s.shift_type)
    if (!w) continue
    const opensAtMs  = istDateMinsToUtcMs(s.shift_date, w.startMins)
    const closesAtMs = istDateMinsToUtcMs(s.shift_date, w.endMins)
    if (closesAtMs <= nowMs) continue   // window fully past — skip
    if (!best || opensAtMs < best.opensAtMs) {
      best = {
        shift_date:  s.shift_date,
        shift_type:  s.shift_type,
        opensAtMs,
        closesAtMs,
        windowStartIst: formatHm(w.startMins),
        windowEndIst:   formatHm(w.endMins),
      }
    }
  }
  return best
