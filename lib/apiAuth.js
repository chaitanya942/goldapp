...
export async function requireAuth(req, opts = {}) {
  ...
  if (!token) {
    return { ok: false, response: Response.json({ error: 'Unauthorized — no session token' }, { status: 401 }) }
  }
  ...
  if (profileErr || !profile) {
    return { ok: false, response: Response.json({ error: 'Unauthorized — no user profile' }, { status: 401 }) }
  }
