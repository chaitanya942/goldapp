...
export async function generateEInvoicePdf({ consignment, branch, items, companySettings, irn, ackNo, ackDt, signedQrCode }) {
  ...
  for (const [l1, v1, l2, v2] of detailRows) {
    ...
    // Auto-shrink each value to its half-column so long numbers (e.g. the full
    // Delivery-Note / Reference number "WGKL/KL-VBP/JUL/2026/004147 dt. 14 Jul 26")
    // never spill across the divider into the next cell.
    drawText(rightColX + 2, dY + 7.5, v1, { size: fitFontSize(v1, halfW - 4, 7.5, 5), bold: true })
    doc.line(rightColX + halfW, dY, rightColX + halfW, dY + detailRowH * 2)
    drawText(rightColX + halfW + 2, dY + 3, l2, { size: 7, color: [80, 80, 80] })
    drawText(rightColX + halfW + 2, dY + 7.5, v2, { size: fitFontSize(v2, halfW - 4, 7.5, 5), bold: true })
  ...
  const renderPartyBlock = ({
    ...
    // pad so the block feels centred. If it OVERFLOWS (long HO address + State
    // line can exceed the box), compress the gaps by `scale` so the block stays
    // inside blockH and never bleeds into the block below / items header.
    const totalH    = stack.reduce((s, e) => s + e.gap, 0) + 2  // 2mm bottom buffer
    const scale     = totalH > blockH ? (blockH - 1) / totalH : 1
    const padTop    = scale < 1 ? 3 : Math.max(0, (blockH - totalH) / 2) + 4
