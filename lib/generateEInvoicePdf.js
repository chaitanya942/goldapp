...
export async function generateEInvoicePdf({ consignment, branch, items, companySettings, irn, ackNo, ackDt, signedQrCode }) {
  ...
  // ── Items table (rendered manually — autoTable plugin doesn't ship here) ──
  const tableW = W - 2 * M
  // column widths summing to tableW
  const cols = [
    { label: '#',          w: 8,  align: 'center' },
    { label: 'Product',    w: 50, align: 'left'   },
    { label: 'HSN',        w: 16, align: 'center' },
    { label: 'Qty',        w: 14, align: 'right'  },
    { label: 'Unit',       w: 10, align: 'center' },
    { label: 'Net Wt',     w: 14, align: 'right'  },
    { label: 'Rate',       w: 14, align: 'right'  },
    { label: 'Assessable', w: 22, align: 'right'  },
    { label: 'GST%',       w: 10, align: 'right'  },
    { label: 'IGST',       w: 14, align: 'right'  },
    { label: 'Total',      w: 14, align: 'right'  },
  ]
  // Scale columns so they fit the available width
  const colTotal = cols.reduce((s, c) => s + c.w, 0)
  const scale    = tableW / colTotal
  ...
  for (const c of cols) {
    const tx = c.align === 'right'  ? cx + c.w - 1
            ...
            : cx + 1
    doc.text(c.label, tx, y + 4.8, { align: c.align })
    cx += c.w
  }
  y += 7
  ...
  const row = [
    '1',
    String(cs.product_name || 'Used Gold Ornaments and Gold Jewellery').slice(0, 32),
  ...
  for (let i = 0; i < cols.length; i++) {
    ...
    const tx = c.align === 'right'  ? cx + c.w - 1
            : c.align === 'center' ? cx + c.w / 2
            : cx + 1
    doc.text(String(row[i]), tx, y + 5.5, { align: c.align })
    // Vertical separator
    if (i < cols.length - 1) doc.line(cx + c.w, y, cx + c.w, y + 8)
    cx += c.w
  }
  y += 8 + 4
