...
export async function generateEInvoicePdf({ consignment, branch, items, companySettings, irn, ackNo, ackDt, signedQrCode }) {
  ...
  // Wrap on whitespace ONLY (never mid-token) so identifiers like the challan
  // number "WGKL/KL-VBP/JUL/2026/004147" stay intact on one line. Shrinks the
  // font until every wrapped line fits maxWidth within maxLines. Used for the
  // doc-detail values (Reference No. wraps to number + "dt. date").
  const wrapNoBreak = (text, maxWidth, maxLines, baseSize, minSize = 5.5) => {
    const words = String(text || '').trim().split(/\s+/).filter(Boolean)
    if (!words.length) return { lines: [], fontSize: baseSize }
    const build = (size) => {
      doc.setFontSize(size)
      const lines = []
      let cur = ''
      for (const w of words) {
        ...
        if (!cur || doc.getTextWidth(trial) <= maxWidth) cur = trial
    ...
    for (let size = baseSize; size >= minSize; size -= 0.25) {
      ...
      if (lines.length <= maxLines && lines.every((l) => doc.getTextWidth(l) <= maxWidth)) {
        return { lines, fontSize: size }
      }
    }
    return { lines: build(minSize).slice(0, maxLines), fontSize: minSize }
  }
  ...
  // Party/doc-details envelope height. Was 80; the Buyer block (label + full HO
  // address + GSTIN + State line ≈ 29 mm) overflowed its share and bled into the
  // items-table header. Taller envelope + the clamp in renderPartyBlock fixes it.
  const partyH = 90
  doc.line(rightColX, y, rightColX, y + partyH)  // vertical divider for parties/doc-details
  ...
  const detailRows = [
    ...
    // Delivery Note + Reference No. both show challan_no (per accounts team —
    // the "reference number" on an e-invoice is the delivery note, not the
    // internal tamper-proof counter that ops uses to track the consignment).
    ['Delivery Note',              consignment.challan_no || consignment.consignment_no || '',  'Mode/Terms of Payment', ''],
    ['Reference No. & Date.',      `${consignment.challan_no || consignment.consignment_no || ''} dt. ${dateStr}`, 'Other References', ''],
    ['Buyer\'s Order No.',         '',                                                          'Dated',                ''],
    ['Dispatch Doc No.',           '',                                                          'Delivery Note Date',   ''],
    ['Dispatched through',         truncFit(consignment.transport_mode || cs.transportation_mode || '', 22),  'Destination', truncFit(cs.head_office_city || 'Bengaluru', 22)],
  ]
  const detailRowH = partyH / detailRows.length / 2
  let dY = y
  const halfW = rightColW / 2
  // Draw a value into its half-cell, wrapping onto up to 2 lines rather than
  // shrinking to a tiny font. The long Reference/Delivery-Note number
  // ("WGKL/KL-VBP/JUL/2026/004147 dt. 14 Jul 26") wraps to two readable lines
  // and stays inside the cell instead of spilling across the divider.
  const drawDetailVal = (x, topY, val) => {
    const { lines, fontSize } = wrapNoBreak(val, halfW - 4, 2, 7.5, 6)
    lines.forEach((ln, i) => drawText(x, topY + i * 3.2, ln, { size: fontSize, bold: true }))
  }
  for (const [l1, v1, l2, v2] of detailRows) {
    drawText(rightColX + 2, dY + 3, l1, { size: 7, color: [80, 80, 80] })
    drawDetailVal(rightColX + 2, dY + 7, v1)
    doc.line(rightColX + halfW, dY, rightColX + halfW, dY + detailRowH * 2)
    drawText(rightColX + halfW + 2, dY + 3, l2, { size: 7, color: [80, 80, 80] })
    drawDetailVal(rightColX + halfW + 2, dY + 7, v2)
    if (dY + detailRowH * 2 < y + partyH) doc.line(rightColX, dY + detailRowH * 2, W - M, dY + detailRowH * 2)
    dY += detailRowH * 2
  }
  ...
  const renderPartyBlock = ({
    ...
    // Distribute the lines down the full block height so a short address (e.g.
    // KL-VENNALA-BY-PASS) fills its box instead of clumping at the top and
    // leaving dead space below. If the content OVERFLOWS (long HO address +
    // State line), compress the gaps so it still stays inside the box.
    const topPad    = 4
    const contentH  = stack.reduce((s, e) => s + e.gap, 0)
    const avail     = blockH - topPad - 1               // usable height below the top pad
    const scale     = contentH > avail ? avail / contentH : 1
    // When there's slack, spread it evenly between lines to fill the box.
    const extraGap  = scale < 1 ? 0 : (avail - contentH) / stack.length

    let cy = blockY + topPad
    for (const e of stack) {
      drawText(M + 2, cy, e.text, { size: e.size, bold: e.bold, color: e.color })
      cy += e.gap * scale + extraGap
    }
  }
  ...
  const buyerH = partyH - sellerH - consigneeH
  renderPartyBlock({
    blockY:      buyerY,
    blockH:      buyerH,
    label:       'Buyer (Bill to)',
  ...
  for (const c of colDef) {
    ...
    // Header labels are centre-aligned in every column for a uniform header row;
    // the data cells keep their own alignment (description left, numbers right).
    for (let i = 0; i < lines.length; i++) {
      drawText(cx + c.w / 2, y + 4 + i * 3.5, lines[i], { size: 7.5, bold: true, align: 'center' })
    }
    cx += c.w
  }
  y += headerH
