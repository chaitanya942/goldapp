...
export async function generateEInvoicePdf({ consignment, branch, items, companySettings, irn, ackNo, ackDt, signedQrCode }) {
  ...
  const detailRows = [
    ['Invoice No.',                truncFit(consignment.consignment_no || consignment.tmp_prf_no, 26), 'Dated', dateStr],
  ...
  for (const [l1, v1, l2, v2] of detailRows) {
    ...
    drawText(rightColX + 2, dY + 7.5, v1, { size: 7.5, bold: true })
    // Right half
  ...
  drawText(M + 2, buyerY + 22.5, `GSTIN/UIN: ${hoGstin}    State Name: ${hoStateName}, Code: ${hoStcd}`, { size: 7, bold: true })
