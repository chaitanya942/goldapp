...
//     Consignment Date | Expected Delivery
//
...
;(() => {
  // Register every Unicode subset of noto-sans we actually use, all under
  // the same family name so @napi-rs/canvas can fall back across them when
  // a glyph isn't in the primary file.
  //   - latin: ASCII + western European
  //   - latin-ext: Indian Rupee Sign (₹, U+20B9), various extended Latin
  //   - greek: Sigma (Σ, U+03A3) used in the totals row label
  // Without the ext + greek subsets, ₹ and Σ rendered as the "missing
  // glyph" tofu box (□) in the PNG export.
...
// The PNG renderer registers a Latin subset of noto-sans, which doesn't
// always pick up U+20B9 (₹) on every deployment of @napi-rs/canvas — the
// glyph rendered as a missing-glyph tofu box in production. Use the plain
// "Rs " prefix here so the export is rock-solid; the on-screen table keeps
// the ₹ symbol via its real browser font.
const fmtAmt   = (n) => n != null ? `Rs ${Number(n).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'
...
export async function generateInTransitJpg({ viewMode = 'case', rows = [], meta = {} } = {}) {
  ...
  // Column widths in px. Sums across must equal W - 2*PAD = 1336.
  // Branch-wise: Branch names are short ("MYSURU", "HOSPETE") so its column
  // doesn't need to dominate. Re-balanced so the numeric columns get more
  // breathing room and the Expected Delivery date column doesn't get hugged
  // by the Amount column to its left.
  const cols = isBranch ? [
    { key: 'consignment_date',       label: 'Consignment Date',  w: 190, align: 'left'  },
    { key: 'branch_name',            label: 'Branch',            w: 220, align: 'left'  },
    ...
    { key: 'total_amount',           label: 'Amount',            w: 246, align: 'right' },
    { key: 'expected_delivery_date', label: 'Expected Delivery', w: 220, align: 'left'  },
  ] : [
    { key: 'application_id',         label: 'App ID',            w: 160, align: 'left'  },
    { key: 'customer_name',          label: 'Customer',          w: 230, align: 'left'  },
    ...
    { key: 'total_amount',           label: 'Amount',            w: 200, align: 'right' },
    { key: 'dispatched_at',          label: 'Consignment Date',  w: 150, align: 'left'  },
    { key: 'expected_delivery_date', label: 'Expected Delivery', w: 136, align: 'left'  },
  ]
  // Cell padding from the left/right edge of each column. Bumped to 18 from
  // 12 so right-aligned headers and the values below them sit clearly inside
  // the column instead of crowding the cell boundary.
  const CELL_PAD = 18
  ...
  let tx = startX
  for (const c of cols) {
    const tpos = c.align === 'right' ? tx + c.w - CELL_PAD : tx + CELL_PAD
    let val = ''
    let color = INK
    if (c.key === cols[0].key) {
      // First cell carries "TOTALS · N rows/bills" — single source of the
      // count so the second column isn't crowded with a label. Drop the
      // leading Σ glyph: it landed as a tofu box in some deployments.
