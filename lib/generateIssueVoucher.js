...
export function generateIssueVoucher({ consignment, sourceBranch, destBranch, companySettings, items, logoBase64 }) {
  ...
  const colW = useW / 4
  doc.setLineWidth(0.2); doc.setDrawColor(...PR)
  for (let i = 1; i < 4; i++) doc.line(L + colW * i, y, L + colW * i, y + valH)

  const valLblY = y + 5
  const valNumY = y + valH * 0.78
  const drawValCell = (label, value, idx, accent = BK) => {
    const cx = L + colW * idx + colW / 2
    doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.setTextColor(...PR)
  ...
  const intraCompanyNote = 'No GST event — intra-company stock transfer. Markup and IGST do not apply for Branch → Hub flows. Per-bill weights and values are confidential and not included on this document.'
  ...
  const disclaimerLines  = doc.splitTextToSize(disclaimer, useW)
  doc.text(disclaimerLines, L, y + 3)
