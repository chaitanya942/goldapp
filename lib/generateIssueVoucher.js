...
export function generateIssueVoucher({ consignment, sourceBranch, destBranch, companySettings, items, logoBase64 }) {
  ...
  // ── SUMMARY BAND — bills count + ONE consolidated gross weight ───────────
  // Replaces the old TRANSFER SUMMARY + VALUE IN WORDS blocks. Two cells
  // side-by-side: total bills (left), consolidated gross weight (right).
  ...
  // Split with a vertical divider in the middle
  ...
  doc.text(String(items.length), L + half / 2, sumMidY + 4, { align: 'center' })

  // Right: CONSOLIDATED GROSS WEIGHT
  doc.setFontSize(7.5); doc.setFont('helvetica', 'bold'); doc.setTextColor(...PR)
  doc.text('CONSOLIDATED GROSS WEIGHT', L + half + half / 2, sumLblY, { align: 'center' })
  doc.setFontSize(13); doc.setFont('helvetica', 'bold'); doc.setTextColor(...BK)
  doc.text(`${totalGross.toFixed(3)} g`, L + half + half / 2, sumMidY + 4, { align: 'center' })

  y += sumH + 3

  // Light disclaimer below the summary so branch knows what it's NOT seeing.
  doc.setFontSize(7); doc.setFont('helvetica', 'italic'); doc.setTextColor(...GY)
  doc.text(
    'No GST event — intra-company stock transfer. Per-bill weights and values are confidential and not included on this document.',
    L, y + 3
  )
  y += 6
