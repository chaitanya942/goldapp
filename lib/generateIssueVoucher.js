...
export function generateIssueVoucher({ consignment, sourceBranch, destBranch, companySettings, items, logoBase64 }) {
  ...
  // Self-carry movements (Rest of Karnataka / AP / TS) name the employee who
  // physically carries the parcel. Ops captures it at consignment creation.
  // Full-width row rather than a half cell — the label plus a person's name
  // doesn't fit the two-column grid above.
  const transporterLabel = (consignment.transporter_name || '').trim()
  if (transporterLabel) {
    const ty2 = y + irH * 0.7
    doc.setDrawColor(...BK); doc.setLineWidth(0.2)
    doc.rect(L, y, half * 2, irH)
    doc.setFontSize(7.5); doc.setTextColor(...BK)
    doc.setFont('helvetica', 'bold');   doc.text('TRANSPORTER', L + 2, ty2)
    doc.setFont('helvetica', 'normal'); doc.text(': ' + transporterLabel, L + irLblW, ty2)
    y += irH
  }
  ...
  const colW = useW / 2
  // `prefill` prints the known value on the rule instead of leaving it blank —
  // the rest of the block is still filled by hand at handover.
  const fieldRow = (label, x, ly, prefill) => {
    ...
    if (prefill) {
      ...
      doc.text(String(prefill), x + 32, ly)
    }
  }
  fieldRow('Carrier Name :',     L,         y + 12, transporterLabel)
