...
export function generateIssueVoucher({ consignment, sourceBranch, destBranch, companySettings, items, logoBase64 }) {
  ...
  // ── AUDIT FINGERPRINT ────────────────────────────────────────────────────
  // Same 8-char fingerprint of this consignment's audited figures, also printed
  // on the Consignee Report and exposed in the document-audit response. It is
  // NOT on the Delivery Challan / EWB / E-Invoice — those don't carry the hash.
  // Matching hash on the Consignee Report = every audited field agrees;
  // a different hash = manual investigation before goods move.
