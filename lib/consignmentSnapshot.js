...
export async function loadConsignmentForGeneration(supabase, consignmentId, auth) {
  ...
  // this consignment's source branch is outside their allowed regions. All
  // current callers (EWB / E-Invoice / Challan / Voucher / Consignee Report
  // generators + previews + document-audit) pass auth — the optional signature
  // is only kept so cron/internal-service callers can still load without auth.
  if (auth) {
  ...
  const branch = mergeSnapshotBranch({
    ...
    snap: {
      ...
      // Per-consignment Branch Contact override. The operator who created
      // the consignment may have named someone other than the branch's
      // default contact (the person actually packing/handing over THIS
      // shipment vs the standing branch manager). Empty/null on the
      // consignment → fall back to live branch defaults inside merge.
      contact_person: consignment.branch_contact_name,
      contact_phone:  consignment.branch_contact_phone,
    },
    live: liveBranch,
  })
  ...
  if (hasSnapshots) {
    ...
    // application_id is NOT a snapshot column (it's a stable per-purchase
    // identifier — doesn't change like weights or addresses might). The new
    // branch-facing docs (Consignee Report, Issue Voucher, Delivery Challan
    // bill list) print it as the primary verification key, so enrich the
    // snapshot items with it from purchases. branch_name is also pulled here
    // because the same docs show 'which branch this bill came from' for hub-
    // consolidation flows where multiple sources end up in one consignment.
    const purchaseIds = items.map(i => i.id).filter(Boolean)
    if (purchaseIds.length > 0) {
      const { data: liveExtras } = await supabase
        .from('purchases')
        .select('id, application_id, branch_name')
        .in('id', purchaseIds)
      const extras = new Map((liveExtras || []).map(p => [p.id, p]))
      items = items.map(i => ({
        ...i,
        application_id: extras.get(i.id)?.application_id || i.application_id || null,
        branch_name:    extras.get(i.id)?.branch_name    || i.branch_name    || consignment.branch_name || null,
      }))
    }
    ...
    items = live || []
    // Ensure branch_name fallback even on the legacy path.
    items = items.map(i => ({ ...i, branch_name: i.branch_name || consignment.branch_name || null }))
  }
...
function mergeSnapshotBranch({ name, snap, live }) {
  return {
    ...
    contact_person: snap.contact_person || live?.contact_person,
    contact_phone:  snap.contact_phone  || live?.contact_phone,
  }
}
