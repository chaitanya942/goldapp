...
export function deriveBranchFields(name, existing) {
  ...
  const regions  = [...new Set(peers.map(b => b.region).filter(Boolean))]
  return {
    // Region drives LOGISTICS (Bangalore = same-day HO; everywhere else = consignment),
    // so only auto-fill it when the prefix's peers unanimously agree. Since the
    // KA- rename, KA- spans BOTH Bangalore and Rest of Karnataka — taking the mode
    // there would silently file a new Mysuru branch as Bangalore/same-day-HO. Leave
    // it blank instead, so the "incomplete data" banner makes ops choose.
    region:     regions.length === 1 ? regions[0] : null,
    // State is still safe to infer: every KA- peer is Karnataka either way.
    state:      mode(peers, 'state'),
    // When the region is ambiguous, default to the CONSERVATIVE model — treating a
    // branch as consignment is recoverable; wrongly marking it same-day-HO is not.
    model_type: regions.length === 1 ? (mode(peers, 'model_type') || 'outside_bangalore') : 'outside_bangalore',
