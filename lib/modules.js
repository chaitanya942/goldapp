...
export const MODULES = [
  {
    ...
    color: '#c9a84c',
    tabs: [
      { id: 'purchase-live',    label: 'Live Data Feed',       dot: '#3aaa6a', desc: 'Real-time walk-ins & conversions' },
      { id: 'purchase-data',    label: 'Master Purchase Data', dot: '#c9a84c', desc: 'Every bill, filter & search' },
      { id: 'purchase-reports', label: 'Purchase Reports',     dot: '#8c5ac8', desc: 'Analytics, trends & intelligence' },
    ],
  },
  {
    id:    'consignments',
    ...
    tabs: [
      ...
      { id: 'consignment-bidding',   label: 'Booking Volume',       dot: '#e58a3b' },
      { id: 'consignment-report',    label: 'Reports',              dot: '#3a8fbf' },
    ],
  },
  // Collection Audit lives downstream of consignments (the bills coming
  // off in-transit get weighed here). Three tabs match the desktop
  // sidebar submenu exactly. Each tab is filtered by canSee on mobile
  // so an `audit` role only sees data + report, while master_auditor
  // sees all three (same DB permission keys as desktop).
  {
    id:    'collection-audit',
    ...
    tabs: [
      ...
      // 'purchase-register' id reused for F&A Reports (Email Reports scheduler).
