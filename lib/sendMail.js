...
function transport() {
  ...
  _transport = nodemailer.createTransport({
    ...
    maxConnections: 3,
    maxMessages: 100,
    // Explicit timeouts are ESSENTIAL with a pool: Gmail silently drops idle
    // keep-alive connections, and without a socket timeout the next send waits
    // forever on the dead half-open socket — the send button hangs on
    // "Sending…" indefinitely (observed 14-Aug-2026: a burst of sends worked,
    // then every send hung once the pooled connection went stale). With these,
    // a dead connection errors within ~30 s so the pool discards it and the
    // caller sees a real failure it can retry, instead of an eternal hang.
    connectionTimeout: 20000,  // TCP connect
    greetingTimeout:   15000,  // wait for the 220 server greeting
    socketTimeout:     30000,  // inactivity on an established socket
  })
  return _transport
}
