...
// Previous COMPLETED calendar week (Monday–Sunday), never the current week.
// Pure string/UTC-date arithmetic — no local-timezone getters — so the
// result is correct regardless of the browser's or server's local timezone
// (unlike `istNow()` + `.getDay()`/`.setDate()`, which double-shifts on an
// IST-local browser since istNow()'s instant already carries the +5:30).
export const istLastWeekRange = () => {
  const [y, m, d] = istToday().split('-').map(Number)
  const today = new Date(Date.UTC(y, m - 1, d))
  const dow = today.getUTCDay()                    // 0 = Sunday
  const off = dow === 0 ? 6 : dow - 1              // days since this week's Monday
  const thisMonday = new Date(today); thisMonday.setUTCDate(thisMonday.getUTCDate() - off)
  const lastSunday = new Date(thisMonday); lastSunday.setUTCDate(lastSunday.getUTCDate() - 1)
  const lastMonday = new Date(lastSunday); lastMonday.setUTCDate(lastMonday.getUTCDate() - 6)
