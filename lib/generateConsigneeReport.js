...
export async function generateConsigneeReport({ consignment, branch, destBranch, items, companySettings = {}, transferHistory = {} }) {
  ...
  // ── 8. AUDIT FINGERPRINT ─────────────────────────────────────────────────
  // Same 8-char hash printed on Issue Voucher / Delivery Challan / EWB /
  // E-Invoice. If two documents in hand show the same hash, they agree on
  // every audited field (addresses, weight, value, markup, IGST, grand
  // total). Diverging hashes = manual investigation BEFORE goods move.
  setFont(13, true)
  ...
  ctx.textAlign = 'right'
  ctx.fillText('cross-check this hash against the Issue Voucher / Delivery Challan / EWB / E-Invoice — they should all match', L + useW - 4, hashMidY)
