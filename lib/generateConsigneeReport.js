...
// numberToWords / amountToWords / fmtIN moved to lib/pdfHelpers.js so all three
// renderers (Issue Voucher, Delivery Challan, Consignee Report) stay in lockstep.
import { amountToWords, fmtIN } from './pdfHelpers'
...
export async function generateConsigneeReport({ consignment, items, companySettings = {}, transferHistory = {} }) {
  ...
  // Interstate iff the goods physically cross state lines.
  // - INTERNAL (Branch → Hub): hub is always in same state as source (enforced
  //   in UI), so always intrastate. No uplift, no IGST.
  // - EXTERNAL Direct→HO / Hub→HO: HO is Karnataka. Interstate iff source != KA.
  const isInternal   = consignment.movement_type === 'INTERNAL'
  const isInterstate = !isInternal && stateCode !== 'KA'
  // Use rates from company_settings (configured in Admin > Company Settings)
  const upliftPct    = parseFloat(companySettings.value_uplift_pct ?? 7.5) / 100
  const igstRate     = parseFloat(companySettings.igst_rate ?? 3) / 100
  const valueOfGoods = isInterstate ? totalAmt * (1 + upliftPct) : totalAmt
  const igst         = isInterstate ? valueOfGoods * igstRate : 0
  ...
  // No internal dividers — single unified header row
  ...
  setFont(FS_EXT, true)
  // Movement-type label reflects the actual consignment type (Internal Branch→Hub
  // moves currently use a separate generator, but if this one is ever invoked for
  // an internal consignment, the label should not lie).
  ...
  // Coerce to string — Branch→Hub consignments don't have challan_no, only tmp_prf_no.
  // Canvas fillText throws on null; coerce so the report still renders.
  ctx.fillStyle = BLUE; ctx.fillText(String(consignment.challan_no || consignment.tmp_prf_no || ''), L + 12 + ctx.measureText(refLabel).width, refMidY)

  const tpX = L + leftColsW * 0.46
  ctx.fillStyle = RED;  ctx.fillText(tpLabel, tpX, refMidY)
  ctx.fillStyle = BLUE; ctx.fillText(String(consignment.tmp_prf_no || ''), tpX + ctx.measureText(tpLabel).width, refMidY)
