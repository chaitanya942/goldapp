...
export async function generateConsigneeReport({ consignment, branch, destBranch, items, companySettings = {}, transferHistory = {} }) {
  ...
  // 6 columns: S.N · DATE · CUSTOMER NAME · APPLICATION ID · BRANCH · GROSS WT
  // Widths in mm — sum must equal useW (281mm).
  // BRANCH is per-bill, not per-consignment: a Hub→HO consolidation carries
  // bills from several source branches, and that's exactly when the packer
  // needs to see which branch each bill came from. The title-bar slot it used
  // to occupy now carries the movement marker instead.
  const CW_MM = [10, 30, 88, 62, 50, 41]
  ...
  setFont(FS_EXT, true)
  // Which document backs this movement, per consignments/route.js:3038 —
  //   INTERNAL (Branch→Hub) -> issue voucher
  //   EXTERNAL (Branch/Hub→HO) -> delivery challan
  // The branch moved back down to its own table column, freeing this slot.
  // cellText (not midText) so the label shrinks to fit rather than bleeding
  // into the date box.
  const movementLabel = consignment?.movement_type === 'INTERNAL' ? '(INTERNAL)' : '(EXTERNAL)'
  cellText(movementLabel, sec2X, y, extMovW, TITLE_H, 'center', WHITE, FS_EXT, true)
  ...
  // ── 2. TABLE HEADER ───────────────────────────────────────────────────────
  // Slim, branch-relevant only: S.N · DATE · CUSTOMER NAME · APPLICATION ID · BRANCH · GROSS WT
  const HEADERS = ['S.N', 'DATE', 'CUSTOMER NAME', 'APPLICATION ID', 'BRANCH', 'GROSS WT']
  ...
  items.forEach((p, idx) => {
    const row = [
      ...
      p.customer_name || '',
      p.application_id || '',
      // Source branch the bill was purchased at — falls back to where it
      // currently sits, then to the consignment's own branch.
      p.branch_name || p.current_branch || consignment?.branch_name || '',
