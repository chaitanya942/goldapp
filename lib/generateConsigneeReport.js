...
export async function generateConsigneeReport({ consignment, branch, destBranch, items, companySettings = {}, transferHistory = {} }) {
  ...
  items.forEach((p, idx) => {
    ...
    row.forEach((val, i) => {
      ...
      // Customer name + application ID left-aligned (longer text reads better
      // against the column edge); gross weight right-aligned so the decimals
      // line up down the column; S.N / DATE stay centered.
      const align = (i === 2 || i === 3) ? 'left' : i === 4 ? 'right' : 'center'
