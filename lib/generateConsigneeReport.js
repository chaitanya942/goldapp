...
export async function generateConsigneeReport({ consignment, items, companySettings = {}, transferHistory = {} }) {
  ...
  // ── 3. DATA ROWS ──────────────────────────────────────────────────────────
  items.forEach((p, idx) => {
    // For transferred-in bills (Hub→HO consolidations), append the prior Branch→Hub
    // TMP PRF so the receiving HO sees provenance. e.g., "KL-EDAPPALLY (via WG000010)"
    const transfer = transferHistory[p.id]
    const branchLabel = transfer && transfer.tmp_prf_no
      ? `${p.branch_name || ''} (via ${transfer.tmp_prf_no})`
    ...
    row.forEach((val, i) => {
      ...
      // Use slightly smaller font for the branch column when it includes "via WG..."
      // so it doesn't overflow the cell
      const fs = (i === 3 && transfer?.tmp_prf_no) ? FS_BODY * 0.85 : FS_BODY
      cellText(val, CX[i], y, CW[i], ROW_H, 'center', BLACK, fs)
    })
    y += ROW_H
  })
