...
export function generateConsigneeReport({ consignment, items }) {
  ...
  // Interstate tax calculation (non-KA branches → Karnataka HO)
  const stateCode    = consignment.state_code || 'KA'
  const isInterstate = stateCode !== 'KA'
  const valueOfGoods = isInterstate ? totalAmt * 1.075 : totalAmt   // +7.5% for interstate
  const igst         = isInterstate ? valueOfGoods * 0.03 : 0       // IGST 3%
  const grandTotal   = valueOfGoods + igst                          // = totalAmt for intrastate
  ...
  // Intrastate (KA): 2 sub-rows — VALUE OF GOODS | GRAND TOTAL
  // Interstate      : 3 sub-rows — VALUE OF GOODS | IGST @ 3% | GRAND TOTAL
  const valRowH  = 7
  const numRows  = isInterstate ? 3 : 2
  const refH     = valRowH * numRows
  const lblW     = 46

  doc.setDrawColor(...BLACK)
  doc.setLineWidth(0.25)

  // Left cell spans full height
  doc.rect(L, tableBottom, leftColsW, refH)

  // Right sub-rows
  for (let i = 0; i < numRows; i++) {
    doc.rect(rightColsStart, tableBottom + i * valRowH, rightColsW, valRowH)
  }
  // Vertical divider inside right cell
  doc.line(rightColsStart + lblW, tableBottom, rightColsStart + lblW, tableBottom + refH)

  // Ref and Tamper — RED bold, vertically centred in left cell
  ...
  const rowY = (i) => tableBottom + i * valRowH + valRowH * 0.72

  // Row 0: VALUE OF GOODS
  doc.text('VALUE  OF GOODS',  rightColsStart + 3,                   rowY(0))
  doc.text(fmtIN(valueOfGoods), rightColsStart + rightColsW - 3, rowY(0), { align: 'right' })

  if (isInterstate) {
    // Row 1: IGST @ 3%
    doc.text('IGST @ 3%',     rightColsStart + 3,                   rowY(1))
    doc.text(fmtIN(igst),      rightColsStart + rightColsW - 3, rowY(1), { align: 'right' })
  }

  // Last row: GRAND TOTAL
  doc.text('GRAND  TOTAL',    rightColsStart + 3,                   rowY(numRows - 1))
  doc.text(fmtIN(grandTotal),  rightColsStart + rightColsW - 3, rowY(numRows - 1), { align: 'right' })
  ...
  doc.setFontSize(9)
  doc.setTextColor(...BLACK)
  doc.text(amountToWords(grandTotal), L + useW / 2, wordsY + 5.2, { align: 'center' })
