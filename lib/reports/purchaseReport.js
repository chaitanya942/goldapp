...
export async function buildPurchaseReport(dateYmd) {
  ...
  // Unified attachment contract (shared with the consignment report + mailer).
  const attachments = [{ filename, contentBase64: xlsxBase64, contentType: XLSX_MIME }]

  return { date: dateYmd, subject, html, filename, xlsxBase64, attachments, counts, errors, isEmpty }
}
