...
//
// TO-HO MOVEMENTS ONLY (movement_type = 'EXTERNAL'). Ops does not want the
// branch → branch/hub leg (movement_type = 'INTERNAL') in the report — that's an
// internal consolidation, not gold arriving at HO, and counting it double-reports
// the same bill. A leaf-branch bill therefore stays out of the report while it
// only sits at the hub; the moment the hub creates its hub → HO consignment,
// EVERY bill in it appears — including the ones transferred in from other
// branches. Each bill is still attributed to its OWN branch (purchases.branch_name),
// not the dispatching hub, so leaf branches keep their own rows.
...
export async function fetchInTransitStock({ from = null, to = null, allowedBranches = null } = {}) {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    let q = supabase
      ...
      .eq('consignments.movement_type', 'EXTERNAL')   // to-HO only — skip branch→hub legs
