...
// ── Generate TMP PRF No (GLOBAL sequential — DB has unique constraint on the column) ──
// TMP PRF is a display number; NIC / IRP no longer see it as the
// DocumentNumber (see buildPayload's docRefForNic helper — that's a unique
// per-attempt fingerprint that doesn't collide on DB resets). So this can
// stay simple and read from the live consignments table.
