...
export async function generateExternalNo(supabase, branchCode, stateCode) {
  ...
  // Skip status='seed' rows — those are per-branch TMP_PRF seeds whose
  // external_no is a placeholder string ('SEED-MYSURU' etc.) that text-sorts
  // above any real numeric '001905' and would freeze the sequence.
  const { data } = await supabase
    .from('consignments')
    .select('external_no')
    .neq('status', 'seed')
