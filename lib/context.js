...
export function AppProvider({ children }) {
  ...
  // ── Kill focus on recharts SVGs so browser never draws a focus ring on click ──
  useEffect(() => {
    const defocus = () => {
      document.querySelectorAll('.recharts-surface, .recharts-wrapper').forEach(el => {
        el.setAttribute('tabindex', '-1')
      })
    }
    defocus()
    const obs = new MutationObserver(defocus)
