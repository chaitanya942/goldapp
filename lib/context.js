...
export const ROLE_PAGES = {
  super_admin:     ['dashboard','purchase-live','purchase-data','purchase-reports','consignment-overview','consignment-data','consignment-report','consignment-summary','consignment-bidding','consignment-analytics','eod-stock-report','audit-data','audit-report','discrepancy-cases','melting','melting-incoming','melting-at-ho','sales','cal-table','live-market-rates','reports','branch-management','branch-employees','user-management','company-settings','logistics','consignment-seeds','import-logs','inbound-bot','role-management','heatmap-insights','consignment-approvals','billed-vs-paid','gold-buying-rate','holiday-calendar'],
  founders_office: ['dashboard','purchase-live','purchase-data','purchase-reports','consignment-overview','consignment-data','consignment-report','consignment-summary','consignment-analytics','eod-stock-report','audit-data','audit-report','melting','melting-incoming','melting-at-ho','sales','cal-table','live-market-rates','reports','branch-management','branch-employees','user-management','company-settings','logistics','consignment-seeds','import-logs','inbound-bot','heatmap-insights','consignment-approvals','billed-vs-paid','gold-buying-rate','holiday-calendar'],
  admin:           ['dashboard','purchase-live','purchase-data','purchase-reports','consignment-overview','consignment-data','consignment-report','consignment-summary','consignment-analytics','eod-stock-report','audit-data','audit-report','cal-table','live-market-rates','heatmap-insights','gold-buying-rate','holiday-calendar'],
  manager:         ['dashboard','purchase-live','purchase-data','purchase-reports','live-market-rates','holiday-calendar'],
  branch_staff:    ['dashboard','purchase-live','purchase-data','purchase-reports','holiday-calendar'],
  viewer:          ['dashboard','purchase-reports','holiday-calendar'],
  telesales:       ['dashboard','inbound-bot'],
  audit:           ['dashboard','audit-data','audit-report','discrepancy-cases'],
}
...
// ── Hard per-page EMAIL allowlist ───────────────────────────────────────────
// Pages listed here are locked to specific emails and override EVERYTHING,
// including super_admin god-mode and any role_permissions config. A page NOT
// listed is unrestricted. Used by both the client (canSeeResolved) and the
// server (requireAuthForPage) so even someone hitting the API/URL directly is
// denied. Emails are matched case-insensitively.
export const PAGE_EMAIL_ALLOWLIST = {
  productivity: ['chaitanya@whitegold.money', 'coral@whitegold.money', 'sreenidhi@whitegold.money'],
}
// The page a permission key belongs to, for the allowlist check.
function pageOfKey(key) {
  if (PAGE_EMAIL_ALLOWLIST[key]) return key
  const head = String(key || '').split('.')
  if ((head[0] === 'tab' || head[0] === 'element') && PAGE_EMAIL_ALLOWLIST[head[1]]) return head[1]
  if (PAGE_EMAIL_ALLOWLIST[head[0]]) return head[0]
  return null
}
export function emailAllowedForPage(email, page) {
  const allow = PAGE_EMAIL_ALLOWLIST[page]
  if (!allow) return true                       // unrestricted page
...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const triggerSync = () => {
      ...
      // Uses authedFetch so the Supabase Bearer token is attached; both endpoints
      // are gated by requireAuth(ADMIN), so a plain fetch() 401s and spams the console.
      authedFetch('/api/sync-purchases?days=2', { method: 'POST' }).catch(() => null)
      authedFetch('/api/sync-new-crm',          { method: 'POST' }).catch(() => null)
    }
  ...
  const resolveElementKey = (key, perms) => {
    ...
    if (key.startsWith('livefeed.')) {
      // Live Feed is its own page now (purchase-live). Keep purchase-data
      // as a fallback for any role still configured the old bundled way.
      return perms.has('page.purchase-live') || perms.has('page.purchase-data')
        || [...perms].some(k => k.startsWith('tab.purchase-data.'))
    }
  ...
  const canSeeResolved = (key) => {
    // ── Hard email allowlist — the SOLE policy for a restricted page, and it
    // overrides EVERYTHING below (super_admin god-mode, role_permissions, the
    // whole lot). Listed email → granted; anyone else → denied (nav hidden,
    // page blocked). Applies to the page key and all its tab/element keys.
    const restrictedPage = pageOfKey(key)
    if (restrictedPage) return emailAllowedForPage(user?.email, restrictedPage)

    // ── super_admin is true god-mode: sees every page / tab / element by
    // design. role_permissions exists to configure OTHER roles; super_admin
    // is intentionally outside that system. The only exception is preview
    // mode (impersonating another role to verify their access), handled
    // immediately below.
    if (role === 'super_admin' && !previewRole) return true
    ...
    if (role === 'super_admin' && previewRole) {
      ...
      if (key.startsWith('livefeed.')) return ROLE_PAGES[pr].includes('purchase-live') || ROLE_PAGES[pr].includes('purchase-data')
  ...
  const canDoResolved = (action) => {
    // super_admin is god-mode for actions too — never subject to DB action
    // permissions, only restricted when previewing another role.
    if (role === 'super_admin' && !previewRole) return true
