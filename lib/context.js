...
export function AppProvider({ children }) {
  ...
  // ── Prevent recharts SVG/wrapper from receiving focus on mouse click ──────────
  // Intercepting mousedown in capture phase and calling preventDefault() stops
  // the browser from assigning focus to the element, so no focus ring ever appears.
  // Tooltips (mousemove-based) and click events are unaffected.
  useEffect(() => {
    const handler = (e) => {
      if (e.target.closest?.('.recharts-wrapper, .recharts-surface')) {
        e.preventDefault()
      }
    }
  ...
  useEffect(() => {
    ...
    const triggerSync = () => {
      // days=2: only last 2 days — keeps the request fast (< 5s) on the free Vercel plan
      Promise.all([
