...
export const ROLE_PAGES = {
  ...
  admin:           ['dashboard','purchase-data','purchase-reports','consignment-overview','consignment-data','consignment-report','consignment-summary','consignment-analytics','cal-table','live-market-rates'],
...
export function AppProvider({ children }) {
  ...
  const canSeeResolved = (key) => {
    // ── Safety: super_admin always retains role-management + user-management access
    // to prevent self-lockout when editing their own permissions.
    if (role === 'super_admin' && !previewRole &&
        (key === 'role-management' || key === 'user-management' ||
         key === 'page.role-management' || key === 'page.user-management')) {
      return true
    }
