...
export async function cancelEInvoice({ irn, reasonCode = '1', remark = 'Duplicate', gstinOverride }) {
  ...
  // ClearTax's pre-NIC validator for IRP cancel enforces NUMERIC CnlRsn,
  // even though NIC's public schema documents CnlRsn as string-of-digit
  // ("1"|"2"|"3"|"4"). Sending the string makes ClearTax come back with
  //   { error_code:"107", error_message:"Invalid value for the field null" }
  // — the same confusingly-worded type-mismatch signature that bit EWB
  // cancel on WG000097 (where ewbNo and cancelRsnCode also had to be coerced
  // to numbers). The "field null" in the message is because their schema-
  // lookup fails before binding the field key, so the diagnostic has no
  // field name to report.
  ...
  const flatPayload    = { Irn: irn, CnlRsn: reasonNum, CnlRem: remark }
  const arrayPayload   = [flatPayload]
  const wrappedPayload = [{ transaction: flatPayload }]

  const attempts = [
    { label: 'flat-put',  method: 'PUT',  body: flatPayload    },
    { label: 'flat-post', method: 'POST', body: flatPayload    },
