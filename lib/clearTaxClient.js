...
export function buildPayload({ consignment, branch, items, destBranch, companySettings }) {
  ...
  // Totals — gross weight in 3 decimals (gold has mg precision; matches the
  // E-Invoice payload, the consignee report, and the items table on challan/
  // voucher). Previously truncated to 2 decimals here, which silently desynced
  // EWB Qty from every other doc whenever the 3rd decimal was non-zero.
  const totalGrossWt = parseFloat(
    (items.reduce((s, it) => s + parseFloat(it.gross_weight || 0), 0)).toFixed(3)
  )
...
export function buildEInvoicePayload({ consignment, branch, items, companySettings }) {
  const HO_DEFAULTS  = resolveHoDefaults(companySettings)
  const stateCode    = getStateCodeFromRegion(branch?.region) || 'KA'
...
export async function generateCreditNote({ originalConsignment, branch, items, companySettings, remark, gstinOverride }) {
  ...
  // For our app this was tmp_prf_no truncated to 16 chars (matching buildEInvoicePayload).
  const originalDocNo = (originalConsignment.tmp_prf_no || originalConsignment.challan_no || '').slice(0, 16)
