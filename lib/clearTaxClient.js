...
export async function cancelEWayBill({ ewbNumber, reasonCode = '1', remark = 'Duplicate Entry', gstinOverride }) {
  ...
  ctaxLog(`EWB cancel ← ${res.status} (${usedShape})`, data)

  // Strict check — NIC MUST return govt_response.Success === 'Y'. The old
  // permissive check (`first?.govt_response && !isGovtSuccess`) treated
  // "200 with no govt_response field" as success, which let local
  // cleanup run while NIC still had the EWB active.
