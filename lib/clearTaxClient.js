...
// ClearTax (4xx) or NIC IRP (govt_response with code 102 = invalid request,
// or 107 = required field missing — both indicate the payload envelope
// reached NIC in the wrong form). We retry with a different shape in that
// case. Any other failure (already-cancelled, IRN out of 24h window, etc.)
// is terminal — never retry.
function isShapeError(httpStatus, govtResponse) {
  if (httpStatus === 405 || httpStatus === 415 || httpStatus === 400) return true
  const errs = govtResponse?.ErrorDetails || govtResponse?.errorDetails || []
  if (Array.isArray(errs)) {
    for (const e of errs) {
      const code = String(e?.error_code || e?.errorCode || e?.ErrorCode || '').trim()
      if (code === '102' || code === '107') return true
    }
  }
  return false
}
...
export async function cancelEInvoice({ irn, reasonCode = '1', remark = 'Duplicate', gstinOverride }) {
  ...
  if (isShapeError(res.status, first?.govt_response)) {
    ctaxLog(`E-Invoice cancel: flat shape rejected (${res.status}), retrying with wrapped`, first?.govt_response || data)
