...
function findErrorCodes(input, found = []) {
  ...
  for (const [k, v] of Object.entries(input)) {
    const lk = k.toLowerCase()
    if (lk === 'errordetails' && Array.isArray(v)) {
      for (const e of v) {
...
export async function cancelEInvoice({ irn, reasonCode = '1', remark = 'Duplicate', gstinOverride }) {
  ...
  // NIC IRP cancel is natively a flat POST: { Irn, CnlRsn, CnlRem }. Some
  // ClearTax tenants on the newer routing accept (and require) the array-
  // wrapped shape that mirrors generate. So we try flat POST first, and
  // ONLY fall back to the wrapped shape if we got a shape/schema error
  // (HTTP 4xx, or NIC error 102 / 107 inside a 200 body).
  //
  // Never retry on 5xx — cancel is destructive and an idempotent retry
  // could mask a real partial-success.
  const flatPayload = { Irn: irn, CnlRsn: reason, CnlRem: remark }
  const wrappedPayload = [{ transaction: flatPayload }]

  let res = await ctaxFetch(url, { method: 'POST', headers, body: JSON.stringify(flatPayload) })
  let data = await res.json().catch(() => ({}))
  let first = Array.isArray(data) ? data[0] : data
  let usedShape = 'flat'

  if (isShapeError(res.status, data)) {
    ctaxLog(`E-Invoice cancel: flat shape rejected (${res.status}), retrying with wrapped`, data)
    // PUT first (newer ClearTax tenants), POST as 405 fallback.
  ...
  ctaxLog(`E-Invoice cancel ← ${res.status} (${usedShape})`, data)

  if (!res.ok || !isGovtSuccess(first?.govt_response) || first?.document_status === 'NOT_CREATED') {
    // Special case: after both shape attempts, NIC still says the IRN is
    // missing/invalid (code 107). This typically means the IRN is no longer
    // active at IRP — either a prior retry already cancelled it or it was
    // never persisted there. Return a soft-success marker so the caller can
    // proceed with local DB cleanup; audit log will record the discrepancy.
...
export async function cancelEWayBill({ ewbNumber, reasonCode = '1', remark = 'Duplicate Entry', gstinOverride }) {
  ...
  const first = Array.isArray(data) ? data[0] : data
  // Strict check — NIC MUST return govt_response.Success === 'Y' for us to
  // accept a cancel. Previous code (`first?.govt_response && !isGovtSuccess`)
  // silently treated "200 OK with no govt_response field" as success, which
  // let local cleanup run while NIC still had the EWB active. Now matches
  // cancelEInvoice's pattern.
  if (!res.ok || !isGovtSuccess(first?.govt_response)) {
    // 107 ("missing EWB") is the one tolerated soft-failure: the EWB
    // genuinely isn't on NIC's books anymore (either already cancelled
    // upstream OR — and this is dangerous — never persisted, or our
    // presented GSTIN doesn't match the one that issued it). The route
    // handler that calls us decides whether to clean up local state or
    // demand manual portal verification.
    if (isIrnGoneAtIrp(data)) {
