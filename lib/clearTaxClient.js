...
export async function cancelEInvoice({ irn, reasonCode = '1', remark = 'Duplicate', gstinOverride }) {
  ...
  // ClearTax pre-NIC validator has shipped at least three accepted shapes
  // over time for the IRP cancel call. We've observed all three failing on
  // different tenants on different days, so try them in series and stop at
  // the first one that NIC accepts. Order is from "most common today" to
  // "documented legacy":
  //   1. Flat POST           : { Irn, CnlRsn, CnlRem }                              // NIC's native IRP contract
  //   2. Array-of-flat PUT   : [{ Irn, CnlRsn, CnlRem }]                            // mirrors generate's array envelope minus transaction wrapper
  //   3. Transaction-wrapped : [{ transaction: { Irn, CnlRsn, CnlRem } }]           // documented legacy
  //
  // For each shape: if HTTP 4xx OR body contains NIC error 102/107 (shape
  // errors), advance to the next shape. Any other failure (already-cancelled,
  // out-of-window, real NIC reject) is terminal — never retry on 5xx, never
  // retry on a non-shape error.
  const flatPayload    = { Irn: irn, CnlRsn: reason, CnlRem: remark }
  const arrayPayload   = [flatPayload]
  const wrappedPayload = [{ transaction: flatPayload }]

  const attempts = [
    { label: 'flat',     method: 'POST', body: flatPayload    },
