...
export async function cancelEWayBill({ ewbNumber, reasonCode = '1', remark = 'Duplicate Entry', gstinOverride }) {
  ...
  if (!res.ok || (first?.govt_response && !isGovtSuccess(first.govt_response))) {
    // Mirror cancelEInvoice: NIC code 107 ("missing EWB") after all retries
    // typically means the EWB is no longer active at NIC (already cancelled
    // upstream or never persisted). Return a soft-success marker so the
    // caller can proceed with local cleanup.
    if (isIrnGoneAtIrp(data)) {
      ctaxLog(`EWB cancel ← treating 107 as already-not-found at NIC`, data)
