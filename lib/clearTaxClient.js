...
function buildPayload({ consignment, branch, items, destBranch, companySettings }) {
  ...
  //  - EXTERNAL (Branch → HO, any state): buyer = HO Bangalore. Even for interstate
  //    own-use transfers, with separate state-wise GSTINs the destination is the KA
  //    HO entity, NOT a copy of the seller. (Old single-GSTIN code copied seller — wrong now.)
  ...
  if (isInternal) {
    ...
    buyerDetails = {
      ...
      Loc:   destBranch?.city || destBranch?.name || '',
      Pin:   normalizePin(destBranch?.pin_code),
      Stcd:  getStateCodeFromGstin(destGstin) || gstinNumForState(destStateCode),
    }
  } else {
    // HO buyer — use the company_settings KA GSTIN if available, fall back to env
