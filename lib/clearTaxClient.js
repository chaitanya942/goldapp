...
export async function cancelEInvoice({ irn, reasonCode = '1', remark = 'Duplicate', gstinOverride }) {
  ...
  // ClearTax's DOCUMENTED cancel schema (docs.cleartax.in .../cancel-irn) is a
  // FLAT object via PUT with a MIXED case that none of the older attempts used:
  //   { irn: <lowercase>, CnlRsn: "<string 1-4>", CnlRem: <string> }
  // i.e. lowercase `irn` but PascalCase `CnlRsn`/`CnlRem`, and CnlRsn as a
  // STRING. Every previous shape paired lowercase `irn` with lowercase reason
  // keys, or PascalCase `Irn` with PascalCase reason keys — so the only attempt
  // that named the reason correctly (`CnlRsn`) also sent `Irn`, which ClearTax
  // can't bind, yielding "Valid Irn number is missing in the request (107)".
  // Try the documented shape FIRST; keep the historical shapes as fallbacks in
  // case a tenant is on an older schema.
  const flatSpec         = { irn, CnlRsn: reason, CnlRem: remark }
  ...
  const attempts = [
    { label: 'flat-spec',          method: 'PUT',  body: flatSpec,       fallbackMethod: 'POST' },
