...
// ── E-Invoice (IRP) ──────────────────────────────────────────────────────────
function buildEInvoicePayload({ consignment, branch, items }) {
  const stateCode    = getStateCodeFromRegion(branch?.region) || 'KA'
  const sellerGstin  = branch?.branch_gstin || WG_GSTIN || ''
  const sellerStcd   = getStateCodeFromGstin(sellerGstin) || gstinNumForState(stateCode)
  const sellerPin    = normalizePin(branch?.pin_code)

  // Buyer = HO (E-Invoice is most relevant for interstate own-use transfers; intra-state
  // same-GSTIN moves don't legally need an E-Invoice but the IRP still accepts them).
  const buyerStcd  = HO_DEFAULTS.Stcd
  const buyerGstin = WG_GSTIN || sellerGstin
...
export async function generateEInvoice({ consignment, branch, items, gstinOverride }) {
  if (!CLEARTAX_URL || !CLEARTAX_TOKEN) throw new Error('ClearTax not configured: set CLEARTAX_URL and CLEARTAX_TOKEN env vars')
  const payload = buildEInvoicePayload({ consignment, branch, items })
  const gstin   = gstinOverride || branch?.branch_gstin || WG_GSTIN
  if (!gstin) throw new Error('No GSTIN available for E-Invoice request')

  const res = await fetch(`${CLEARTAX_URL}/einv/v2/eInvoice/generate`, {
    method: 'POST',
    headers: {
      'Content-Type':          'application/json',
      'X-Cleartax-Auth-Token': CLEARTAX_TOKEN,
  ...
  if (!res.ok || data?.govt_response?.Success === 'N') {
    throw new Error(extractClearTaxMessage(data, 'E-Invoice generation failed'))
  }
  return data
}
