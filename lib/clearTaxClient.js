...
function buildPayload({ consignment, branch, items, companySettings }) {
  ...
  // NIC auto-calculates distance from PIN codes when seller and buyer PINs differ.
  // When PINs are same (or one is missing), NIC throws error 107 — so we send a
  // safe fallback. 1km is the minimum NIC accepts.
  const buyerPin    = HO_DEFAULTS.Pin
  const samePin     = sellerPin && buyerPin && Number(sellerPin) === Number(buyerPin)
  const transDistance = samePin ? 1 : undefined
  ...
  if (transDistance) payload.TransDistance = transDistance
  return payload
}
