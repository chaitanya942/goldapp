...
function buildPayload({ consignment, branch, items, destBranch, companySettings }) {
  ...
  //  - OUTWARD interstate own-use (Branch → HO): buyer = same entity at origin state's GSTIN.
  //  - INWARD intra-state KA: buyer = HO (Bangalore Koramangla).
  ...
  // NIC auto-calculates distance from PINs but rejects when PINs are missing,
  // identical, or unrecognised. Always send TransDistance with a safe estimate
  // so the request is never blocked. NIC accepts ±10% deviation from real distance.
  const buyerPin = buyerDetails?.Pin
  const samePin  = sellerPin && buyerPin && Number(sellerPin) === Number(buyerPin)
  // Heuristic: same state ~ 50km, interstate ~ 500km, same PIN/INTERNAL hub ~ 1km
  const sellerStcdNum = Number(sellerStcd) || 29
  const buyerStcdNum  = Number(buyerDetails?.Stcd) || 29
  const transDistance = samePin
    ? 1
    : (sellerStcdNum === buyerStcdNum ? 50 : 500)
...
function buildEInvoicePayload({ consignment, branch, items, companySettings }) {
  ...
  // Tax computation: gold HSN 71131910 attracts 3% GST.
  // Inter-state B2B (different state GSTINs): IGST 3%
  // Intra-state B2B (same state, different GSTINs — rare): CGST 1.5% + SGST 1.5%
  // The receiving branch claims ITC. Even for OWN_USE deemed supply, IRP requires tax.
  const buyerStcd0  = HO_DEFAULTS.Stcd
  const isInterstate = sellerStcd !== buyerStcd0
  const gstRate      = 3
  ...
  const raw = {
    ...
    SellerDtls: {
      Gstin: sellerGstin,
      LglNm: companySettings?.company_name || 'White Gold Bullion pvt ltd',
      TrdNm: companySettings?.company_name || 'White Gold Money',
      Addr1: branch?.address || '',
      Loc:   branch?.city || branch?.name || '',
      Pin:   sellerPin,
      Stcd:  sellerStcd,
    ...
    BuyerDtls: {
      Gstin: buyerGstin,
      LglNm: companySettings?.company_name || HO_DEFAULTS.LglNm,
      TrdNm: companySettings?.company_name || HO_DEFAULTS.TrdNm,
      Pos:   buyerStcd,
