...
export function buildEInvoicePayload({ consignment, branch, items, companySettings }) {
  ...
  // Unique-per-row reference for IRP — same fingerprint format as the EWB
  // payload so both NIC and IRP see the same identifier and the audit hash
  // they ship with stays consistent. Avoids the 2026-05-07 stale-return bug.
  const docNo   = buildNicDocRef(consignment)
...
export async function generateCreditNote({ originalConsignment, branch, items, companySettings, remark, gstinOverride }) {
  ...
  // Must match buildEInvoicePayload exactly so IRP can find the precedent doc.
  const originalDocNo = buildNicDocRef(originalConsignment)
