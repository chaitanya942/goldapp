...
function buildPayload({ consignment, branch, items, destBranch, companySettings }) {
  ...
  const sellerDetails = {
    ...
    TrdNm: companySettings?.trade_name   || companySettings?.company_name || HO_DEFAULTS.TrdNm,
...
// ClearTax exposes a print endpoint that returns the IRP-signed PDF (with QR).
export async function fetchEInvoicePdf({ irn, gstinOverride }) {
  if (!CLEARTAX_URL || !CLEARTAX_TOKEN) throw new Error('ClearTax not configured')
  if (!irn) throw new Error('IRN required to fetch E-Invoice PDF')
  const gstin = gstinOverride || WG_GSTIN

  // Try multiple URL patterns since ClearTax docs vary by version
  const candidates = [
    `${CLEARTAX_URL}/einv/v2/eInvoice/print?irn=${encodeURIComponent(irn)}`,
    `${CLEARTAX_URL}/einv/v2/eInvoice/print/${encodeURIComponent(irn)}`,
    `${CLEARTAX_URL}/einv/v2/eInvoice/${encodeURIComponent(irn)}/print`,
  ]
  let lastErr = null
  for (const url of candidates) {
    try {
      ...
      if (!res.ok) {
        ...
        lastErr = `${url} → ${res.status} ${txt.slice(0, 120)}`
        continue
      }
      ...
      // Some endpoints return a JSON wrapper with a base64 PDF
      const j = await res.json().catch(() => null)
      const b64 = j?.data || j?.pdf || j?.base64 || j?.invoice
      if (b64 && typeof b64 === 'string') {
        return Buffer.from(b64, 'base64')
      }
      lastErr = `${url} → unexpected content-type ${ctype}`
    } catch (err) {
