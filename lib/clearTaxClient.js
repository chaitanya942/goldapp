...
export async function generateCreditNote({ originalConsignment, branch, items, companySettings, remark, gstinOverride }) {
  ...
  // Credit note: change document type + reference the original.
  // Document number must be unique → prefix CN- to the original number.
  const originalDocNo   = base.DocDtls?.No || originalConsignment.tmp_prf_no || originalConsignment.challan_no
  const originalDocDate = base.DocDtls?.Dt
  base.DocDtls = {
    ...base.DocDtls,
    Typ: 'CRN',
