-- ─────────────────────────────────────────────────────────────────────────────
-- One-off: mark all Bangalore-branch bills purchased on 16 Jun 2026 as received
-- at HO (stock_status → 'at_ho').
--
-- Scope:
--   • branch region = 'Bangalore'   (derived from the branches table, not hardcoded)
--   • purchase_date = '2026-06-16'   (this date only)
...
SELECT p.crm_source, p.stock_status, p.crm_status,
 ...
 WHERE b.region        = 'Bangalore'
   AND p.purchase_date = '2026-06-16'
...
-- ── STEP 3 · VERIFY — approved Bangalore 16-Jun bills still not at_ho (expect 0)
