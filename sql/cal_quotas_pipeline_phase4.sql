-- ─────────────────────────────────────────────────────────────────────────────
-- Pipeline Phase 4 — auto-attacher skips stale (past-arrival) bookings
-- ─────────────────────────────────────────────────────────────────────────────
-- Bug fixed here: process_pipeline_attachments() had no guard against
-- bookings whose arrival_date is already in the past. So a Kerala booking
-- placed yesterday, with pipeline still owed, kept getting back-filled by
-- TODAY's leaf→hub bills — stealing today's stock for a dead booking.
--
-- Fix: the attacher now only fills bookings whose pipeline_arrival_date is
-- today or later. A past-arrival booking is left untouched — close_stale_
-- pipelines() converts its leftover pipeline straight to realized gain
-- (which is exactly what the operator asked: "at midnight whatever is in
-- the pipeline, consider that as gain").
--
-- sync-purchases also reorders the two so close runs BEFORE attach — belt
-- and braces — but this date filter is the real fix.
--
...
BEGIN
  ...
  FOR booking IN
    ...
    WHERE q.pipeline_remaining_g > 0
      ...
      AND q.pipeline_arrival_date >= today_ist          -- NEW: never fill a stale (past-arrival) booking
