...
export default function LiveMarketRates() {
  ...
  const [aamlinRate,    setAamlinRate]    = useState(null)   // live from browser socket
  const [aamlinStatus,  setAamlinStatus]  = useState('connecting') // connecting | live | error

  const kRef       = useRef(null)
  const aRef       = useRef(null)
  const mRef       = useRef(null)
  const tRef       = useRef(null)
  const aamlinSave = useRef(null) // ref to save interval

  // ── Aamlin Browser Socket ─────────────────────────────────────────────────
  useEffect(() => {
    let socket = null
    let lastSavedRate = null

    const connectAamlin = async () => {
      try {
        const { io } = await import('socket.io-client')
        setAamlinStatus('connecting')

        socket = io('http://starlinebulltech.in:10001', {
        ...
        socket.on('disconnect', () => setAamlinStatus('connecting'))
        socket.on('connect_error', () => setAamlinStatus('error'))

      } catch (e) {
        console.error('[Aamlin] Import error:', e.message)
        setAamlinStatus('error')
      }
    }

    connectAamlin()

    // Save Aamlin rate to Supabase every minute (update latest row)
    aamlinSave.current = setInterval(async () => {
      if (!lastSavedRate) return
      try {
        // Get the latest gold_rates row and update its aamlin_sell_rate
        ...
        if (latest?.id) {
          await supabase
            .from('gold_rates')
            .update({ aamlin_sell_rate: lastSavedRate })
            .eq('id', latest.id)
          console.log('[Aamlin] Saved to Supabase:', lastSavedRate)
        }
      } catch (e) { console.error('[Aamlin] Save error:', e.message) }
    }, 60000)

    return () => {
      if (socket) socket.disconnect()
      if (aamlinSave.current) clearInterval(aamlinSave.current)
    }
  }, [])
  ...
  // Use live Aamlin rate from browser socket if available, else fall back to DB
  const liveAamlinRate = aamlinRate || latest?.aamlin_sell_rate

  const spread_ka = latest?.kalinga_sell_rate && latest?.ambica_sell_rate ? latest.kalinga_sell_rate - latest.ambica_sell_rate : null
  const spread_km = latest?.kalinga_sell_rate && liveAamlinRate ? latest.kalinga_sell_rate - liveAamlinRate : null

  const card = { background: t.card, border: `1px solid ${t.border}`, borderRadius: '10px' }

  const aamlinStatusColor = aamlinStatus === 'live' ? t.green : aamlinStatus === 'connecting' ? t.orange : t.red
  const aamlinStatusLabel = aamlinStatus === 'live' ? 'Live' : aamlinStatus === 'connecting' ? 'Connecting...' : 'Unavailable'

  const RateCard = ({ label, sub, rate, s, color, canvasRef, statusDot }) => (
    <div style={{ ...card, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '13px 16px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: '11px', color, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}>
            {label}
            {statusDot && <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: statusDot.color, display: 'inline-block' }} title={statusDot.label} />}
          </div>
    ...
    <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 10px', borderRadius: '20px', background: `${t.green}15`, border: `1px solid ${t.green}35` }}>
      ...
      {latest && (
        <div style={{ ...card, padding: '10px 18px', background: t.card2, display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
          {[
            ...
            { label: 'AAMLIN SPOT',   rate: liveAamlinRate,           change: aamlinRate ? aamlinRate - (latest?.aamlin_sell_rate || aamlinRate) : mStat.change, color: t.purple },
          ].map(item => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
        ...
        <RateCard label="Aamlin Spot"         sub="Gold 999 IND"                  rate={liveAamlinRate}             s={mStat} color={t.purple} canvasRef={mRef}
          statusDot={{ color: aamlinStatusColor, label: aamlinStatusLabel }} />
      </div>
        ...
        <div style={{ maxHeight: '260px', overflowY: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              {todayRates.length === 0 && (
                <tr><td colSpan={9} style={{ padding: '28px', textAlign: 'center', color: t.text4, fontSize: '12px' }}>No data yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
