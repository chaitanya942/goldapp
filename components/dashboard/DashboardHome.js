...
// (e.g. 91.5% → 92.0% read as "+0.5%") reads as a much bigger move than it
// is; the point difference (+0.5pp) is what's actually meaningful here.
function fmtPointChange(curr, prev) {
  if (curr == null || prev == null) return null
  const d = Number(curr) - Number(prev)
  return `${d >= 0 ? '+' : ''}${d.toFixed(2)}pp`
}
// Absolute change for a plain count (e.g. active branches) — a branch count
// moving 40→45 reads more directly as "+5" than "+12.5%".
...
function ConsignmentBalanceView({ t, stats, isMobile, setActiveNav }) {
  ...
  // to both Branch In Stock and In Transit at once, re-aggregating from the
  // per-(branch, purchase_date) breakdown rather than re-fetching, so
  // switching presets is instant.
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{
      ...
      {!loading && prevValue != null && (
        <div style={{ marginTop: sub && !compact ? 8 : 10, paddingTop: 8, borderTop: `1px dashed ${t.border}`, display:'flex', flexDirection:'column', gap: 3, position:'relative', zIndex: 1 }}>
    ...
    if (canSee('consignment-report')) {
      ps.push(
        Promise.all([
          ...
          // applies Bangalore's time-of-day lifecycle automatically:
          //   < 19:30 IST → today's approved bills are at_branch
          //   19:30+ IST  → today's approved bills are in_consignment
          //   next day    → previous day's bills become at_ho (drop off both views)
