...
function PurchaseInline({ t, setActiveNav, canSee }) {
  ...
  useEffect(() => { fetchPeriod() }, [period, selectedState, selectedRegion]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const fetchPeriod = async () => {
    ...
    if (selectedRegion) {
      ...
      const statesInReg = statesInRegion
      const filteredStates = (allStates.data||[]).filter(s => statesInReg.includes(s.state))
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      {(availableRegions.length > 0 || availableStates.length > 0) && (
