...
const T = {
  dark: {
    ...
    goldBdr:    'rgba(201,168,76,.22)',
    goldSdw:    '0 0 12px rgba(201,168,76,.08), inset 0 1px 0 rgba(201,168,76,.1)',
    goldGlow:   'rgba(201,168,76,.5)',
    border:     '#1a1a18',
    border2:    '#222018',
    hov:        'rgba(201,168,76,.06)',
    topGlow:    'radial-gradient(ellipse at top, rgba(201,168,76,.06) 0%, transparent 60%)',
    divider:    'linear-gradient(90deg, transparent, rgba(201,168,76,.15), transparent)',
    sectionClr: '#2a1e0e',
    logoBg:     'linear-gradient(135deg, #c9a84c 0%, #7a4a10 100%)',
  ...
  light: {
    ...
    goldBdr:    'rgba(160,120,48,.25)',
    goldSdw:    '0 0 8px rgba(160,120,48,.08), inset 0 1px 0 rgba(160,120,48,.1)',
    goldGlow:   'rgba(160,120,48,.4)',
    border:     '#d0c8b8',
    border2:    '#c8c0b0',
    hov:        'rgba(160,120,48,.06)',
    topGlow:    'radial-gradient(ellipse at top, rgba(160,120,48,.05) 0%, transparent 60%)',
    divider:    'linear-gradient(90deg, transparent, rgba(160,120,48,.18), transparent)',
    sectionClr: '#b0a080',
    logoBg:     'linear-gradient(135deg, #a07830 0%, #6a3a08 100%)',
...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
      ...
      <div key={item.id} style={{ marginBottom: '1px' }}>
        ...
        >
          <div style={{
            ...
            flexShrink: 0,
            filter: active ? `drop-shadow(0 0 5px ${t.goldGlow})` : 'none',
          ...
          {sidebarOpen && (
            <>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontSize: '.76rem', fontWeight: active ? 600 : 400,
                  color: active ? t.gold : t.text3,
                  letterSpacing: '.03em', lineHeight: 1.3,
                  transition: 'color .18s',
                }}>{item.label}</div>
                {!item.children && (
                  <div style={{ fontSize: '.55rem', color: t.text4, marginTop: '1px', letterSpacing: '.05em' }}>{item.desc}</div>
                )}
              </div>
              {item.children && (
                <div style={{
                  ...
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={active ? t.gold : t.text3} strokeWidth="2.5" strokeLinecap="round">
        ...
        {item.children && expanded && sidebarOpen && (
          <div style={{
            marginLeft: '24px', paddingLeft: '16px',
            borderLeft: `1px solid ${t.goldBdr}`,
            ...
            {item.children.map(child => (
              <div
                ...
                style={{
                  ...
                  background: isActive(child.id) ? `${child.dot || t.gold}18` : 'transparent',
              ...
              >
                <div style={{
                  ...
                  background: isActive(child.id) ? (child.dot || t.gold) : t.text4,
                  boxShadow: isActive(child.id) ? `0 0 6px ${child.dot || t.gold}` : 'none',
                  transition: 'all .18s',
                }} />
                <span style={{
                  fontSize: '.72rem',
                  color: isActive(child.id) ? (child.dot || t.gold) : t.text3,
  ...
  return (
    <div style={{
      width: sidebarOpen ? '232px' : '52px', flexShrink: 0,
      background: t.side,
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: t.logoBg, borderRadius: '9px',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: t.logoSdw,
        }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 0', scrollbarWidth: 'none', position: 'relative', zIndex: 1 }}>
        {sidebarOpen && (
          <div style={{ fontSize: '.5rem', color: t.sectionClr, letterSpacing: '.2em', textTransform: 'uppercase', padding: '4px 20px 8px', fontWeight: 600 }}>Main</div>
        )}
        {visibleNavItems.map(renderItem)}

        {/* Admin section — only shown if role has at least one admin page */}
        {visibleAdminItems.length > 0 && (
          <>
            <div style={{ margin: '10px 16px', height: '1px', background: t.divider }} />
            {sidebarOpen && (
              <div style={{ fontSize: '.5rem', color: t.sectionClr, letterSpacing: '.2em', textTransform: 'uppercase', padding: '4px 20px 8px', fontWeight: 600 }}>Admin</div>
            )}
      ...
      >
        <div style={{
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={t.text3} strokeWidth="2.5" strokeLinecap="round"
            style={{ transform: sidebarOpen ? 'none' : 'rotate(180deg)', transition: 'transform .22s' }}>
