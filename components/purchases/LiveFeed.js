...
const STAGE_META = {
  WALKIN:                  { label: 'Walk-in',         color: '#3a8fbf', order: 0 },
  ESTIMATION_PENDING:      { label: 'Valuation',        color: '#c9981f', order: 1 },
  KYC_PENDING:             { label: 'KYC',              color: '#8c5ac8', order: 2 },
  FINAL_PAYMENT_PENDING:   { label: 'Payment',          color: '#c9a84c', order: 3 },
  FINAL_PAYMENT_COMPLETED: { label: 'Purchased ✓',      color: '#3aaa6a', order: 4 },
  WALKOUT:                 { label: 'Walkout',          color: '#e05555', order: 5 },
}
...
export default function LiveFeed() {
  ...
  const [tlFilter,    setTlFilter]    = useState('')   // timeline filter
  ...
  // ── Stage funnel (new CRM) ───────────────────────────────────────────────────
  const stageOrder = ['WALKIN', 'ESTIMATION_PENDING', 'KYC_PENDING', 'FINAL_PAYMENT_PENDING', 'FINAL_PAYMENT_COMPLETED']
  const maxStageCount = stages
    ? Math.max(1, ...stageOrder.map(s => stages[s]?.count || 0))
    : 1
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      <div style={{ ...card, padding: '16px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1px', background: t.border }}>
          {[
            { label: 'Walked In',    wt: walkedInWt,   count: walkinSummary.total || 0,        color: t.blue,   icon: '→' },
            { label: 'In Progress',  wt: pendingWt,    count: Number(summary.pending  || 0),   color: t.orange, icon: '⏳' },
            { label: 'Purchased',    wt: purchasedWt,  count: Number(summary.approved || 0),   color: t.green,  icon: '✓' },
            { label: 'Walkouts',     wt: rejectedWt,   count: Number(summary.rejected || 0) + Number(walkinSummary.visited_not_sold || 0), color: t.red, icon: '✕' },
          ].map((p, i) => (
      ...
      {/* ── STAGE FUNNEL (new CRM) + KPI CARDS ────────────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: stages ? '1.2fr 1fr' : '1fr', gap: '14px' }}>
        ...
        {stages && (
          <div style={{ ...card, padding: '16px 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
              {stageOrder.map(key => {
                ...
                return (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '72px', fontSize: '.62rem', color: meta.color, fontWeight: 600, flexShrink: 0 }}>{meta.label}</div>
                    <Bar pct={pct} color={meta.color} height={6} />
                    <div style={{ width: '28px', fontSize: '.72rem', color: t.text1, textAlign: 'right', fontFamily: 'monospace', flexShrink: 0 }}>{d.count}</div>
                    {d.net_wt > 0 && (
                      <div style={{ width: '52px', fontSize: '.6rem', color: t.text3, textAlign: 'right', flexShrink: 0 }}>{d.net_wt.toFixed(1)}g</div>
                    )}
                  </div>
                )
              })}
              {/* Walkouts separately */}
              {stages.WALKOUT && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', borderTop: `1px solid ${t.border}`, paddingTop: '7px', marginTop: '2px' }}>
                  <div style={{ width: '72px', fontSize: '.62rem', color: t.red, fontWeight: 600, flexShrink: 0 }}>Walkout</div>
                  <Bar pct={(stages.WALKOUT.count / maxStageCount) * 100} color={t.red} height={6} />
                  <div style={{ width: '28px', fontSize: '.72rem', color: t.red, textAlign: 'right', fontFamily: 'monospace', flexShrink: 0 }}>{stages.WALKOUT.count}</div>
                </div>
              )}
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', alignContent: 'start' }}>
          {[
            { label: 'Walk-ins Today',   value: fmtNum(walkinSummary.total || 0),    color: t.blue,   sub: walkedInWt > 0 ? `${walkedInWt.toFixed(1)}g gold` : null },
            { label: 'Bills Submitted',  value: fmtNum(summary.total || 0),          color: t.text1,  sub: `${summary.branches_active || 0} branches` },
            { label: 'Approved',         value: fmtNum(summary.approved || 0),       color: t.green,  sub: purchasedWt > 0 ? `${purchasedWt.toFixed(2)}g` : null },
            ...
            <div key={k.label} style={{ ...card, padding: '12px 14px' }}>
              ...
              <div style={{ fontSize: '1.2rem', fontWeight: 200, color: k.color, fontFamily: 'monospace' }}>{k.value}</div>
      ...
      {/* ── BRANCH BREAKDOWN + PAYMENT SPLIT ──────────────────────────────── */}
