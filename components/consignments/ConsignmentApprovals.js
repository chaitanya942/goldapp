...
export default function ConsignmentApprovals() {
  ...
  useEffect(() => {
    if (tab === 'cancellations')           fetchCancellations()
    else if (tab === 'reports')            fetchReport(reportFrom, reportTo)
    else if (tab === 'settings')           fetchSettings()
    else                                   fetchHistory(tab)
  }, [tab, fetchHistory, fetchCancellations, fetchReport, fetchSettings, reportFrom, reportTo])
    ...
    <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: '10px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <button onClick={() => {
            if (tab === 'cancellations')         fetchCancellations(false)
...
function ReportsTab({ t, card, report, reportFrom, setReportFrom, reportTo, setReportTo, fetchReport, showToast }) {
  const [downloadingId, setDownloadingId] = useState(null)
  const downloadRow = async (r, docType) => {
    const key = `${r.id}:${docType}`
    setDownloadingId(key)
    try {
      const url = docType === 'ewb' ? `/api/eway-bill/pdf?id=${r.id}` : `/api/e-invoice/pdf?id=${r.id}`
      await previewDoc(url, docFilename({ consignment: r, docType, ext: 'pdf' }), msg => showToast?.(msg, 'error'))
    } finally {
  ...
  const exportEwbs = () => downloadCsv(`EWB_${dateTag}.csv`, report.ewbs.map(r => ({
    ...
    Movement:             r.is_interstate ? 'Interstate' : 'Intrastate',
    ...
    IGST:                 r.igst_amount != null ? Number(r.igst_amount).toFixed(2) : '',
          ...
          <tbody>
            {report.ewbs.map((r, i) => (
              <ReportRow key={r.id} t={t} striped={i % 2 === 1}>
                ...
                <td style={td}>
                  <span style={{
                    fontSize: '9.5px', fontWeight: 700, letterSpacing: '.04em', borderRadius: '5px', padding: '2px 7px',
                    color: r.is_interstate ? t.purple : t.text3,
                    background: r.is_interstate ? `${t.purple}18` : `${t.text3}12`,
                  }}>
                    {r.is_interstate ? 'INTERSTATE' : 'INTRASTATE'}
                  </span>
                </td>
                ...
                <td style={{ ...td, textAlign: 'center' }}>
                  <button onClick={() => downloadRow(r, 'ewb')} disabled={downloadingId === `${r.id}:ewb`}
                    title="Download the EWB PDF"
                    style={{
                      background: 'transparent', border: `1px solid ${t.green}50`, color: t.green, borderRadius: '6px',
                      padding: '4px 10px', fontSize: '10.5px', fontWeight: 700, cursor: downloadingId ? 'wait' : 'pointer',
                      opacity: downloadingId === `${r.id}:ewb` ? 0.5 : 1,
                    }}>
                    {downloadingId === `${r.id}:ewb` ? '…' : '⬇ PDF'}
                  </button>
                </td>
              </ReportRow>
            ))}
            ...
            {report.einvoices.map((r, i) => {
              ...
              return (
                <ReportRow key={r.id} t={t} striped={i % 2 === 1}>
                  ...
                  <td style={td}>
                    <span style={{ fontSize: '9.5px', fontWeight: 700, letterSpacing: '.04em', borderRadius: '5px', padding: '2px 7px', color: t.purple, background: `${t.purple}18` }}>
                      INTERSTATE
                    </span>
                  </td>
                  <td style={{ ...td, color: t.text2, textAlign: 'right' }}>{r.total_bills}</td>
                  ...
                  <td style={{ ...td, color: t.text3, fontSize: '10px' }}>{r.generated_by || '—'}</td>
                  <td style={{ ...td, textAlign: 'center' }}>
                    <button onClick={() => downloadRow(r, 'einvoice')} disabled={downloadingId === `${r.id}:einvoice`}
                      title="Download the E-Invoice PDF"
                      style={{
                        background: 'transparent', border: `1px solid ${t.purple}50`, color: t.purple, borderRadius: '6px',
                        padding: '4px 10px', fontSize: '10.5px', fontWeight: 700, cursor: downloadingId ? 'wait' : 'pointer',
                        opacity: downloadingId === `${r.id}:einvoice` ? 0.5 : 1,
                      }}>
                      {downloadingId === `${r.id}:einvoice` ? '…' : '⬇ PDF'}
                    </button>
                  </td>
                </ReportRow>
              )
